...
import (
	...
	"github.com/byndsuite/byndd/addrmgr"
	...
	"github.com/byndsuite/byndd/mining/cpuminer"
	"github.com/byndsuite/byndd/netsync"
	...
	"github.com/byndsuite/byndutil/bloom"
)
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a byndutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := byndutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a byndutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := byndutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (sp *serverPeer) OnFeeFilter(_ *peer.Peer, msg *wire.MsgFeeFilter) {
	...
	if msg.MinFee < 0 || msg.MinFee > byndutil.MaxSatoshi {
		peerLog.Debugf("Peer %v sent an invalid feefilter '%v' -- "+
			"disconnecting", sp, byndutil.Amount(msg.MinFee))
		sp.Disconnect()
		return
	}
...
func (s *server) TransactionConfirmed(tx *byndutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			bynddLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new byndd server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, bynddLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *byndutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
