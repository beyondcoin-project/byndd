...
// newAddressPubKey returns a new byndutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) byndutil.Address {
	addr, err := byndutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new byndutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) byndutil.Address {
	addr, err := byndutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new byndutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) byndutil.Address {
	addr, err := byndutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the byndutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the byndutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := byndutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := byndutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := byndutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := byndutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := byndutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*byndutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*byndutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*byndutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*byndutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*byndutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
