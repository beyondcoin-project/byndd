...
var (
	bynddHomeDir           = byndutil.AppDataDir("byndd", false)
	byndctlHomeDir         = byndutil.AppDataDir("byndctl", false)
	byndwalletHomeDir      = byndutil.AppDataDir("byndwallet", false)
	defaultConfigFile     = filepath.Join(byndctlHomeDir, "byndctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(bynddHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(byndwalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for byndctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(byndctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		...
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(byndwalletHomeDir, "byndwallet.conf")
		} else {
			serverConfigPath = filepath.Join(bynddHomeDir, "byndd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either byndd or
// byndwallet), and extract the RPC user and password from it.
