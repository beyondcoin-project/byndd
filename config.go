...
import (
	...
	"github.com/byndsuite/byndd/connmgr"
	"github.com/byndsuite/byndd/database"
	_ "github.com/byndsuite/byndd/database/ffldb"
	"github.com/byndsuite/byndd/mempool"
	"github.com/byndsuite/byndd/peer"
	"github.com/byndsuite/byndutil"
)

const (
	defaultConfigFilename        = "byndd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "byndd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-byndd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = byndutil.AppDataDir("byndd", false)
...
// config defines the configuration options for byndd.
//
...
//
// The above results in byndd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		bynddLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = byndutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]byndutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := byndutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		bynddLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-byndd.conf to the given destination path,
...
// bynddDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func bynddDial(addr net.Addr) (net.Conn, error) {
...
// bynddLookup resolves the IP of the given host using the correct DNS lookup
...
func bynddLookup(host string) ([]net.IP, error) {
