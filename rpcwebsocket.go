...
import (
	...
	"github.com/byndsuite/byndd/database"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *byndutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *byndutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *byndutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a byndutil.Address) {
	switch a := a.(type) {
	case *byndutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *byndutil.AddressScriptHash:
		f.scriptHashes[*a.Hash160()] = struct{}{}
		return
	case *byndutil.AddressPubKey:
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := byndutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a byndutil.Address) bool {
	switch a := a.(type) {
	case *byndutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a byndutil.Address) {
	switch a := a.(type) {
	case *byndutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected byndutil.Block
type notificationBlockDisconnected byndutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *byndutil.Tx,
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *byndutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *byndutil.Tx) {
	...
	ntfn := btcjson.NewTxAcceptedNtfn(txHashStr, byndutil.Amount(amount).ToBTC())
	marshalledJSON, err := btcjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*byndutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *byndutil.Block, txIndex int) *btcjson.BlockDetails {
	if block == nil {
		return nil
	}
...
func newRedeemingTxNotification(txHex string, index int, block *byndutil.Block) ([]byte, error) {
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *byndutil.Tx, block *byndutil.Block) {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *byndutil.Tx,
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *byndutil.Tx, block *byndutil.Block) {
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := byndutil.DecodeAddress(addr, params)
		if err != nil {
...
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *byndutil.Block) {
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *byndutil.Block, params *chaincfg.Params) []string {
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *byndutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *btcjson.RescanCmd, lookups *rescanKeys, minBlock,
	...
	*byndutil.Block, *chainhash.Hash, error) {
