...
type memWallet struct {
	...
	addrs map[uint32]byndutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]byndutil.Address)
...
// SetRPCClient saves the passed rpc connection to byndd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*byndutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          byndutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (byndutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []byndutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt byndutil.Amount,
	feeRate byndutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := byndutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate byndutil.Amount) (*chainhash.Hash, error) {
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	feeRate byndutil.Amount, change bool) (*wire.MsgTx, error) {
	...
	var outputAmt byndutil.Amount
	for _, output := range outputs {
		outputAmt += byndutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() byndutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance byndutil.Amount
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (byndutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := byndutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
