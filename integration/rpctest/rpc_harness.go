...
// Harness fully encapsulates an active byndd process to provide a unified
// platform for creating rpc driven integration tests involving byndd. The
// active byndd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active byndd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*byndutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the byndd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []byndutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure byndd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created byndd
...
func (h *Harness) NewAddress() (byndutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() byndutil.Amount {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*byndutil.Tx, blockVersion int32,
	blockTime time.Time) (*byndutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*byndutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*byndutil.Block, error) {
	...
	prevBlock := byndutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "byndd", "rpctest")
