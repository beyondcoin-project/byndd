...
import (
	...
	rpc "github.com/byndsuite/byndd/rpcclient"
	"github.com/byndsuite/byndutil"
)

// nodeConfig contains all the args, and data required to launch a byndd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	bynddPath, err := btcdExecutablePath()
	if err != nil {
		bynddPath = "byndd"
	}
...
// arguments returns an array of arguments that be used to launch the byndd
// process.
...
// command returns the exec.Cmd which will be used to start the byndd process.
...
// to the byndd process that is launched via Start().
...
// as the base for the log and data directories for byndd.
...
// start creates a new byndd process, and writes its pid in a file reserved for
...
// stop interrupts the running byndd process process, and waits until it exits
...
// shutdown terminates the running byndd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := byndutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
