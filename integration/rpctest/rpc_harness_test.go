...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt byndutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(byndutil.Amount(5 * byndutil.SatoshiPerBitcoin))
	...
	txid = genSpend(byndutil.Amount(500 * byndutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(byndutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*byndutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, byndutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := byndutil.Amount(250 * byndutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = byndutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := byndutil.Amount(50 * byndutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := byndutil.Amount(numMatureOutputs * 50 * byndutil.SatoshiPerBitcoin)
