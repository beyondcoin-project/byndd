...
import (
	...
	"github.com/byndsuite/byndd/blockchain"
	"github.com/byndsuite/byndd/blockchain/indexers"
	"github.com/byndsuite/byndd/btcjson"
	"github.com/byndsuite/byndd/chaincfg"
	"github.com/byndsuite/byndd/chaincfg/chainhash"
	"github.com/byndsuite/byndd/mining"
	"github.com/byndsuite/byndd/txscript"
...
type Config struct {
	...
	FetchUtxoView func(*byndutil.Tx) (*blockchain.UtxoViewpoint, error)
	...
	// utxo view.
	CalcSequenceLock func(*byndutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type TxPool struct {
	...
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*byndutil.Tx
	outpoints     map[wire.OutPoint]*byndutil.Tx
...
func (mp *TxPool) removeOrphan(tx *byndutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *byndutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *byndutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*byndutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *byndutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *byndutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *byndutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *byndutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *byndutil.Tx, height int32, fee int64) *TxDesc {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *byndutil.Tx) (bool, error) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) signalsReplacement(tx *byndutil.Tx,
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txAncestors(tx *byndutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*byndutil.Tx) map[chainhash.Hash]*byndutil.Tx {

	// If a cache was not provided, we'll initialize one now to use for the
	// recursive calls.
	if cache == nil {
		cache = make(map[chainhash.Hash]map[chainhash.Hash]*byndutil.Tx)
	}

	ancestors := make(map[chainhash.Hash]*byndutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txDescendants(tx *byndutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*byndutil.Tx) map[chainhash.Hash]*byndutil.Tx {
	...
	descendants := make(map[chainhash.Hash]*byndutil.Tx)
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txConflicts(tx *byndutil.Tx) map[chainhash.Hash]*byndutil.Tx {
	conflicts := make(map[chainhash.Hash]*byndutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
		conflict, ok := mp.outpoints[txIn.PreviousOutPoint]
		if !ok {
			continue
...
func (mp *TxPool) CheckSpend(op wire.OutPoint) *byndutil.Tx {
	mp.mtx.RLock()
	txR := mp.outpoints[op]
	mp.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *byndutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*byndutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) validateReplacement(tx *byndutil.Tx,
	txFee int64) (map[chainhash.Hash]*byndutil.Tx, error) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *byndutil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	txHash := tx.Hash()
	...
	var conflicts map[chainhash.Hash]*byndutil.Tx
	if isReplacement {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *byndutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *byndutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*byndutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *byndutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
func (mp *TxPool) RawMempoolVerbose() map[string]*btcjson.GetRawMempoolVerboseResult {
	...
	for _, desc := range mp.pool {
		...
		mpd := &btcjson.GetRawMempoolVerboseResult{
			...
			Fee:              byndutil.Amount(desc.Fee).ToBTC(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev:  make(map[wire.OutPoint]map[chainhash.Hash]*byndutil.Tx),
		nextExpireScan: time.Now().Add(orphanExpireScanInterval),
		outpoints:      make(map[wire.OutPoint]*byndutil.Tx),
	}
}
