...
import (
	...
	"github.com/byndsuite/byndd/btcec"
...
func (s *fakeChain) FetchUtxoView(tx *byndutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *byndutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *byndutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   byndutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*byndutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee byndutil.Amount,
	signalsReplacement bool) (*byndutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput byndutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*byndutil.Tx, error) {
	txChain := make([]*byndutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, byndutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := byndutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *byndutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee byndutil.Amount,
	signalsReplacement, confirmed bool) *byndutil.Tx {
...
func testPoolMembership(tc *testContext, tx *byndutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
	txHash := tx.Hash()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   byndutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *byndutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*byndutil.Tx, []*byndutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*byndutil.Tx, []*byndutil.Tx) {
				...
				return tx, []*byndutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*byndutil.Tx, []*byndutil.Tx) {
				...
				return tx, []*byndutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = byndutil.SatoshiPerBitcoin
