...
const (
	...
	DefaultMinRelayTxFee = byndutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee byndutil.Amount) int64 {
	...
	if minFee < 0 || minFee > byndutil.MaxSatoshi {
		minFee = byndutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *byndutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee byndutil.Amount) bool {
...
func checkTransactionStandard(tx *byndutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee byndutil.Amount,
...
func GetTxVirtualSize(tx *byndutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
