...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-byndd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "byndd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of byndd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-byndd.conf")
