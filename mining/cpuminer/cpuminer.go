...
type Config struct {
	...
	ProcessBlock func(*byndutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *byndutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), byndutil.Amount(coinbaseTx.Value))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, curHeight+1, ticker, quit) {
			block := byndutil.NewBlock(template.Block)
			m.submitBlock(block)
		}
	}
