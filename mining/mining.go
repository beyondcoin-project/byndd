...
const (
	...
	MinHighPriority = byndutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr byndutil.Address) (*byndutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *byndutil.Tx, height int32) error {
...
func logSkippedDeps(tx *byndutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress byndutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*byndutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := byndutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := byndutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A byndutil.Block should use saved in the state to avoid
