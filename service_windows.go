...
const (
	// svcName is the name of byndd service.
	svcName = "bynddsvc"
...
// logServiceStartOfDay logs information about byndd when the main server has
...
// bynddService houses the main service handler which handles all service
// updates and launching bynddMain.
type bynddService struct{}
...
// long-running bynddMain (which is the real meat of byndd), handles service
// change requests, and notifies the service control manager of changes.
func (s *bynddService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start bynddMain in a separate goroutine so the service can start
...
// installService attempts to install the byndd service.  Typically this should
...
// removeService attempts to uninstall the byndd service.  Typically this should
...
// startService attempts to start the byndd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &bynddService{})
	if err != nil {
