...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *byndutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// Deprecated: Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *byndutil.Tx, details *btcjson.BlockDetails)
	...
	// result of a rescan request, due to how byndd may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount byndutil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance byndutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case btcjson.BtcdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warnf("Received invalid byndd connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a byndd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*byndutil.Tx, error) {
	...
	transactions := make([]*byndutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = byndutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*byndutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return byndutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := byndutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseBtcdConnectedNtfnParams parses out the connection status of byndd
// and btcwallet from the parameters of a bynddconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance byndutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := byndutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a byndd extension and requires a websocket connection.
...
//
// Deprecated: Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []byndutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// Deprecated: Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []byndutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []byndutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a byndd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []byndutil.Address,
...
// NOTE: This is a byndd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []byndutil.Address, outPoints []wire.OutPoint) error {
