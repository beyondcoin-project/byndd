...
Bitcoin RPC server that uses a byndd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with byndd (https://github.com/byndsuite/byndd),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, byndd and
...
byndd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by byndd and
...
All notifications provided by byndd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by byndd (and
...
Also, it is important to realize that byndd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the byndd RPC server directly, only the RPCs which are related to
...
The following full-blown client examples are in the examples directory:
 ...
 - bynddwebsockets
   Connects to a byndd RPC server using TLS-secured websockets, registers for
