...
byndd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, byndd supports a hybrid,
...
Configuring byndd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the byndd command
line or in the byndd configuration file.  Typically the Tor proxy address will be
...
By default, byndd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since byndd will act as the server.
...
```text
HiddenServiceDir /var/tor/byndd
HiddenServicePort 9333 127.0.0.1:9333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring byndd as a Tor hidden service requires
three flags:
...
```bash
$ ./byndd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
byndd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the byndd command line or in the byndd
...
```bash
$ ./byndd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
byndd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./byndd --proxy=127.0.0.1:9050 --torisolation
```
