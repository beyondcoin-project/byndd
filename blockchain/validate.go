...
const (
	...
	baseSubsidy = 50 * byndutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *byndutil.Tx) bool {
...
func IsFinalizedTransaction(tx *byndutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *byndutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if satoshi > byndutil.MaxSatoshi {
		...
		if totalSatoshi > byndutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *byndutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *byndutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *byndutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *byndutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *byndutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *byndutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *byndutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *byndutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *byndutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *byndutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", byndutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > byndutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				byndutil.Amount(originTxSatoshi),
				byndutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > byndutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *byndutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *byndutil.Block) error {
	b.chainLock.Lock()
