...
func addrToKey(addr byndutil.Address) ([addrKeySize]byte, error) {
	...
	case *byndutil.AddressWitnessScriptHash:
		var result [addrKeySize]byte
		...
		// push to 20-bytes.
		copy(result[1:], byndutil.Hash160(addr.ScriptAddress()))
		return result, nil

	case *byndutil.AddressWitnessPubKeyHash:
		var result [addrKeySize]byte
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*byndutil.Tx
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *byndutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *byndutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *byndutil.Block,
	stxos []blockchain.SpentTxOut) error {
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr byndutil.Address,
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *byndutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*byndutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *byndutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr byndutil.Address) []*byndutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*byndutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*byndutil.Tx),
