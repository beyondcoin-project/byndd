...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: byndutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := byndutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*byndutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, byndutil.NewTx(tx))
	}
...
func additionalCoinbase(amount byndutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee byndutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee byndutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee byndutil.Amount) *wire.MsgTx {
